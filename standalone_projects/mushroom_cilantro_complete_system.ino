/*
 * RDTRC Complete Mushroom & Cilantro Growing System - Standalone Version
...
#define SYSTEM_NAME "Mushroom & Cilantro Growing System"
#define DEVICE_ID "RDTRC_MUSHROOM_CILANTRO"
...
#define BLYNK_DEVICE_NAME "RDTRC Growing System"
...
void setup() {
  ...
  Serial.println("✅ RDTRC Mushroom & Cilantro Growing System Ready!");
...
void displayBootScreen() {
  Serial.println("\n" + String("=").repeat(60));
  Serial.println("🍄🌿 RDTRC Complete Mushroom & Cilantro Growing System");
  Serial.println("");
...
void setupSystem() {
  ...
  sendLineNotification("🍄🌿 RDTRC Growing System Started!\n"
...
void setupWebServer() {
  ...
  server.on("/api/phase", HTTP_POST, []() {
    ...
    } else if (zone == "cilantro" && phase.length() > 0) {
...
void handleManualControls() {
  ...
  if (digitalRead(RESET_BUTTON_PIN) == LOW) {
    if (!resetButtonPressed) {
      ...
      sendLineNotification("🔄 Growing system reset initiated");
      delay(1000);
      ESP.restart();
    }
  } else {
...
<html>
<head>
    <title>RDTRC Growing System</title>
    <meta charset="UTF-8">
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .zones-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .zone-card { background: #1a1f2e; border-radius: 10px; padding: 20px; border: 1px solid #2d3748; }
        .zone-card.mushroom { border-left: 4px solid #8e44ad; }
        .zone-card.cilantro { border-left: 4px solid #2ecc71; }
        ...
        .control-item label { margin-bottom: 5px; color: #a0aec0; font-weight: 500; font-size: 12px; }
        .control-item input, .control-item select { padding: 8px; border: 1px solid #4a5568; border-radius: 5px; background: #2d3748; color: #e6e6e6; }
    ...
    <script>
        ...
        function createZoneContent(zone, type) {
            const icon = type === 'mushroom' ? '🍄' : '🌿';
            const color = type === 'mushroom' ? '#8e44ad' : '#2ecc71';
            
            return `
                <div class="zone-header">
                    <h3>${icon} ${zone.name}</h3>
                ...
                <div class="environmental-controls">
                    ${type === 'mushroom' ? `
                        <button class="btn ${zone.watering_active ? 'btn-success' : 'btn-secondary'}" onclick="toggleControl('${type}', 'misting')">
                            ${zone.watering_active ? '💧 Misting ON' : '💧 Misting OFF'}
                        </button>
                        <button class="btn ${zone.heating_active ? 'btn-danger' : 'btn-secondary'}" onclick="toggleControl('${type}', 'heating')">
                            ${zone.heating_active ? '🔥 Heat ON' : '🔥 Heat OFF'}
                        </button>
                        <button class="btn ${zone.fan_active ? 'btn-primary' : 'btn-secondary'}" onclick="toggleControl('${type}', 'fan')">
                            ${zone.fan_active ? '💨 Fan ON' : '💨 Fan OFF'}
                        </button>
                    ` : `
                        <button class="btn ${zone.watering_active ? 'btn-success' : 'btn-secondary'}" onclick="toggleControl('${type}', 'watering')">
                            ${zone.watering_active ? '💧 Water ON' : '💧 Water OFF'}
                        </button>
                        <button class="btn ${zone.light_active ? 'btn-warning' : 'btn-secondary'}" onclick="toggleControl('${type}', 'light')">
                ...
                <div style="margin-top: 15px;">
                    <button class="btn btn-secondary" onclick="configureZone('${type}')">⚙️ Configure</button>
                    <button class="btn btn-secondary" onclick="changePhase('${type}')">🔄 Change Phase</button>
                </div>
            `;
        }
        ...
        function toggleControl(zone, action) {
            ...
            const currentState = document.querySelector(`#${zone}-card button`).textContent.includes('ON');
            const newState = !currentState;
            
            fetch('/api/control', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `zone=${zone}&action=${action}&value=${newState}`
            })
            .then(response => response.json())
            .then(data => {
        ...
        function configureZone(zone) {
            const temp = prompt(`Enter target temperature for ${zone} (°C):`, zone === 'mushroom' ? '22' : '20');
            const humidity = prompt(`Enter target humidity for ${zone} (%):`, zone === 'mushroom' ? '85' : '60');
            const moisture = prompt(`Enter target moisture for ${zone} (%):`, zone === 'mushroom' ? '70' : '50');
            
            if (temp && humidity && moisture) {
                fetch('/api/settings', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `${zone}_temp=${temp}&${zone}_humidity=${humidity}&${zone}_moisture=${moisture}`
                })
                .then(response => response.json())
                .then(data => {
        ...
        function changePhase(zone) {
            const phases = zone === 'mushroom' 
                ? ['Inoculation', 'Colonization', 'Pinning', 'Fruiting']
                : ['Germination', 'Seedling', 'Vegetative', 'Harvest'];
            
            const phase = prompt(`Select growth phase for ${zone}:\\n${phases.join('\\n')}`, phases[0]);
            
            if (phase && phases.includes(phase)) {
                fetch('/api/phase', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `zone=${zone}&phase=${phase}`
                })
                .then(response => response.json())
                .then(data => {
...
<body>
    ...
    <div class="header">
        <h1>🍄🌿 RDTRC Mushroom & Cilantro Growing System</h1>
        <p>Complete Standalone Dual-Zone Growing System - Firmware by RDTRC v4.0</p>
    </div>
    ...
    <div class="container">
        ...
        <h2 style="margin-bottom: 20px; color: #8e44ad;">🌱 Growing Zones</h2>
        <div class="zones-grid">
            <div class="zone-card mushroom" id="mushroom-card">
                <!-- Mushroom zone content will be loaded here -->
            </div>
            <div class="zone-card cilantro" id="cilantro-card">
        ...
        <div class="controls">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                <div>
                    <p><strong>Device ID:</strong> RDTRC_MUSHROOM_CILANTRO</p>
                    <p><strong>Firmware Version:</strong> 4.0</p>
                    <p><strong>System Type:</strong> Dual-Zone Growing System</p>
